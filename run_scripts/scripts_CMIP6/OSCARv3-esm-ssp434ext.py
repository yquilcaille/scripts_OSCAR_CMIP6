...
## 3. INITIALIZATION
##################################################
## Using initialization from last year of ssp434
out_init = xr.open_dataset('results/'+folder+'/esm-ssp434_Out-'+str(setMC)+'.nc' )
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'esm-ssp434ext':
## - Concentrations for CH4, N2O and halo: from 'concentrations_ScenarioMIP', SSP4-3.4-ext
## *********************************************************
...
## *********************************************************
## - LULCC from LUH2, extension for SSP4-3.4
...
for_runs_scen = xr.open_dataset('results/'+folder+'/esm-ssp434_For-'+str(setMC)+'.nc')
...
with xr.open_dataset('input_data/drivers/concentrations_ScenarioMIP.nc') as TMP:
    ...
    for var in ['CH4','N2O','Xhalo']:
        val = TMP[var].loc[{'year':np.arange(year_start+1,year_end+1),'scen':'SSP4-3.4'}] - Par[var+'_0']
...
if SOURCE_EXTENSION=='ONeill2017': # O'Neill et al, 2017 (doi:10.5194/gmd-9-3461-2016)
    ...
    for var in ['E_BC','E_CO','E_NH3','E_VOC','E_NOX','E_OC','E_SO2'] + ['E_CH4']:
        ## non-CO2 emissions from LU AND FF&I stay constant after 2100
        cst_emi = TMP[var].loc[dict(scen='SSP4-3.4',year=2100,sector=[sec for sec in TMP.sector.values if sec not in nonCO2_ignored_sectors])].sum('sector')
    ...
    cst_emi = TMP['E_N2O'].loc[dict(scen='SSP4-3.4',year=2100)]
    ...
    if TMP['Eff'].loc[dict(scen='SSP4-3.4',year=2100,sector=[sec for sec in TMP.sector.values if sec in sectors_LU])].sum() > 0.: raise Exception("Check definition of the sectors.")
    todec_emiFF = TMP['Eff'].loc[dict(scen='SSP4-3.4',year=2100,sector=[sec for sec in TMP.sector.values if sec not in nonCO2_ignored_sectors])].sum('sector')
    ...
    for var in ['E_BC','E_CO','E_NH3','E_VOC','E_NOX','E_OC','E_SO2'] + ['E_CH4']:
        ## non-CO2 emissions from LU stay constant after 2100
        cst_emiLU = TMP[var].loc[dict(scen='SSP4-3.4',year=2100,sector=sectors_LU)].sum('sector')
        For[var] = For[var].fillna( cst_emiLU )
        ## non-CO2 emissions from FF&I decrease linearly to  0 over 2100-2250
        todec_emiFF = TMP[var].loc[dict(scen='SSP4-3.4',year=2100,sector=[sec for sec in TMP.sector.values if sec not in nonCO2_ignored_sectors+sectors_LU])].sum('sector')
    ...
    todec_emiall = TMP['E_N2O'].loc[dict(scen='SSP4-3.4',year=2100)]
    ...
    For['Eff'].loc[dict(year=range(year_start,dico_dates_end_plateau['SSP4-3.4']+1))] = todec_emiFF # constant over 2100-2140
    For['Eff'].loc[dict(year=np.arange(dico_dates_end_plateau['SSP4-3.4'],dico_dates_end_ramp['SSP4-3.4']+1))] = np.linspace( todec_emiFF , 0.*todec_emiFF , dico_dates_end_ramp['SSP4-3.4']-dico_dates_end_plateau['SSP4-3.4']+1)  # linear ramp over 2140-2190
    For['Eff'].loc[dict(year=np.arange(dico_dates_end_ramp['SSP4-3.4']+1,year_end+1))] = 0. # 0 afterwards
...
for var in ['d_Hwood','d_Ashift',]:
    For[var].loc[{'year':np.arange(year_start,year_end+1)}] = For0L[var].loc[{'year':2099,'scen_LULCC':'SSP4-3.4'}] # constant extension
