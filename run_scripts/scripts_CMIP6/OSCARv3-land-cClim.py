...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'land-cClim':
## - Concentration for CO2: concentrations CMIP6
## - Climatology (local temperatures and precipitations): constant climate, using recycling GSWP3 1901-2010 over full period
...
with xr.open_dataset('input_data/observations/climatology_GSWP3.nc') as TMP:
    ...
    for var in ['Tl','Pl']:
        ...
        For['D_'+var].loc[{'year':np.arange(1861,2000+1)}] = np.stack( list(val.sel(year=np.arange(1901,1920+1)).values) * int((2000-1860)/20.) )
        For['D_'+var].loc[{'year':np.arange(2001,2010+1)}] = val.sel(year=np.arange(1901,1910+1)).values
