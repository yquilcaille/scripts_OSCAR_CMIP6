...
name_experiment = 'ssp245-stratO3'
year_PI = 1850
year_start = 2020
year_end = 2100
nMC = 1000
...
out_init = xr.open_dataset('results/'+folder+'/hist-stratO3_Out-'+str(setMC)+'.nc' )
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'ssp245-stratO3' (similar to piControl, except D_O3s):
...
## - Stratospheric ozone forced to outputs of OSCAR from 'ssp245' over 2020-2100
...
for_runs_hist = xr.open_dataset('results/'+folder+'/hist-stratO3_For-'+str(setMC)+'.nc' )

## Preparing variables
for var in for_runs_hist.variables:
    if (var not in for_runs_hist.coords):
        if (var in ['D_O3s']):
            pass ## will be defined entirely later
        else:
...
For['D_O3s'] = out_scen.D_O3s.sel(year=np.arange(year_start,year_end+1))
