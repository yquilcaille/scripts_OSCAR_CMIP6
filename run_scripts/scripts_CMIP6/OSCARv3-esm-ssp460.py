...
##################################################
## Forcings for 'ssp460':
## - Concentrations for CH4, N2O and halo: from 'concentrations_ScenarioMIP', SSP4-6.0
## - Emissions for all from 'emissions_ScenarioMIP', SSP4-6.0  (N2O, CH4 and Xhalo: not used, but here to allow the computation to run). Eff fall in this category
## - LULCC from LUH2, SSP4-6.0
...
for var in ['E_BC','E_CO','E_NH3','E_VOC','E_NOX','E_OC','E_SO2'] + ['Eff','E_CH4','E_N2O']:
    For[var].loc[{'year':np.arange(year_start+1,year_end+1)}] = For0E[var].loc[{'year':np.arange(year_start+1,year_end+1),'scen_'+var:'SSP4-6.0'}]
...
For['d_Hwood'].loc[{'year':np.arange(year_start+1,year_end),'bio_land':For0L.bio_land}] = For0L['d_Hwood'].loc[{'year':np.arange(year_start+1,year_end),'scen_LULCC':'SSP4-6.0'}]
For['d_Hwood'].loc[dict(year=year_end)] = For['d_Hwood'].sel(year=year_end-1) # 2100 as 2099
for var in ['d_Ashift','d_Acover']:
    For[var].loc[{'year':np.arange(year_start+1,year_end),'bio_from':For0L.bio_from,'bio_to':For0L.bio_to}] = For0L[var].loc[{'year':np.arange(year_start+1,year_end),'scen_LULCC':'SSP4-6.0'}]
