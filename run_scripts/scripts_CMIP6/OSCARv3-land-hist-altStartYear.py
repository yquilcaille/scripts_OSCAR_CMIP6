...
name_experiment = 'land-hist-altStartYear'
year_PI = 1700
year_start = 1700
year_end = 2010
nMC = 500
...
out_init = xr.open_dataset('results/'+folder+'/land-spinup-altStartYear_Out-'+str(setMC)+'.nc' )
## Because of recycling over 1700-1900 of the period 1901-1920, and matching, need the last equivalent of a 1920
ind_ini = out_init.year.size - (out_init.year.size % 20) - 1
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'land-hist-altStartYear':
## - Concentration for CO2: concentrations CMIP6
## - Climatology (local temperatures and precipitations): GSWP3 recycled over 1901-1920 for 1700-1900, then GSWP3 1901-2010
## - Land-use: LUH2 historical
...
with xr.open_dataset('input_data/observations/climatology_GSWP3.nc') as TMP:
    ...
    for var in ['Tl','Pl']:
        ...
        ## recycling 1901-1920 over 1700-1900
        if (year_start != 1700) or (year_end != 2010):
            raise Exception("This script has been prepared for a period 1700-2010.")
        For['D_'+var].loc[{'year':np.arange(1701,1900+1)}] = np.stack( list(val.sel(year=np.arange(1901,1920+1)).values) * int((1900-1700)/20.) )
        For['D_'+var].loc[{'year':1700}] = val.sel(year=1920)
...
for_init = xr.open_dataset('results/'+folder+'/land-spinup-altStartYear_For-'+str(setMC)+'.nc' )
