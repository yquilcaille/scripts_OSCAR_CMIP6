...
name_experiment = '1pctCO2-cdr'
year_PI = 1850
year_start = 1850
year_end = 1850 + 2*140+1000
nMC = 1000
setMC = 1
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for '1pctCO2-cdr':
## - Concentrations for CO2:
##      - from the value in 1850, increase by 1%/yr of the atmospheric CO2 (1850: CO2_1850, 1851: *1.01, 1852: *1.01**2.,...) over 140 years
##      - decrease by 1%/yr of the atmospheric CO2 over 140 years
##      - remain constant to 1850 CO2 over 1000 year
...
with xr.open_dataset('input_data/observations/concentrations_CMIP6.nc') as TMP:
    ...
    vals = xr.DataArray(  np.cumprod( np.concatenate([np.repeat(1.01,140), np.repeat( 1/1.01 ,140), np.ones(year_end-year_start-2*140)]) )  ,dims=('year'),coords={'year':np.arange(year_start+1,year_end+1)} )
    For['D_CO2'].loc[{'year':np.arange(year_PI+1,year_end+1)}] = vals * TMP['CO2'].loc[{'year':year_PI,'region':'Globe'}] - Par['CO2_0']
