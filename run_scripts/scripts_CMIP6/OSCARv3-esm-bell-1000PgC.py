...
name_experiment = 'esm-bell-1000PgC'
year_PI = 1850
year_start = 1850
year_end = 1850+100+1000
nMC = 1000
setMC = 1
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'esm-bell-1000PgC':
## - Concentrations for CH4, N2O and halo from 'concentrations_CMIP6', year 1850
## - Emissions of CO2 from 'extra_data/ZECMIP_data/ZEC-MIP_Bellcurve_1000PgC.csv'
...
## Eff
For['Eff'] = xr.DataArray( np.zeros((year_end-year_start+1,For.reg_land.size)), dims=('year','reg_land') )
TMP = np.array( [line for line in csv.reader(open('extra_data/ZECMIP_data/ZEC-MIP_Bellcurve_1000PgC.csv'))][5:] , dtype=np.float32)
For['Eff'].loc[{'reg_land':0,'year':np.arange(year_start+1,year_start+1+len(TMP))}] = TMP[:,1] # affecting these emissions from 1851 so that they will be accounted over whole period
