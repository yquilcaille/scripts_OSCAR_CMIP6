...
out_init = xr.open_dataset('results/'+folder+'/ssp434_Out-'+str(setMC)+'.nc' )
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'ssp434ext':
## - Concentrations for CO2, CH4, N2O and halo: from 'concentrations_ScenarioMIP', SSP4-3.4-ext
## *********************************************************
...
for_runs_scen = xr.open_dataset('results/'+folder+'/ssp434_For-'+str(setMC)+'.nc')
...
if SOURCE_EXTENSION=='ONeill2017': # O'Neill et al, 2017 (doi:10.5194/gmd-9-3461-2016)
    ...
    For['Eff'].loc[dict(year=np.arange(2140+1,2185+1))] = np.linspace(todec_emiFF,xr.DataArray(np.zeros(shape=todec_emiFF.shape),dims=todec_emiFF.dims),2185-2140+1)[1:]  # linear ramp over 2140-2185
    For['Eff'].loc[dict(year=np.arange(2185+1,year_end+1))] = 0. # 0 afterwards
