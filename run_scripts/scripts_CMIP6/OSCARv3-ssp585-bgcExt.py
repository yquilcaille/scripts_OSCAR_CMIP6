...
## 3. INITIALIZATION
##################################################
## Using initialization from last year of ssp585-bgc
out_init = xr.open_dataset('results/'+folder+'/ssp585-bgc_Out-'+str(setMC)+'.nc' )
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'ssp585-bgcExt':
## - Concentrations for CO2, CH4, N2O and halo: from 'concentrations_ScenarioMIP', SSP5-8.5-ext
...
## Using dataset from forcings to prepare those from this experiment
for_runs_scen = xr.open_dataset('results/'+folder+'/ssp585_For-'+str(setMC)+'.nc')
...
if SOURCE_EXTENSION=='ONeill2017': # O'Neill et al, 2017 (doi:10.5194/gmd-9-3461-2016)
    ...
    For['Eff'].loc[dict(year=range(2100,dico_dates_end_plateau['SSP5-8.5']+1))] = todec_emiFF # constant over 2100-2140
    ...
    For['Eff'].loc[dict(year=np.arange(dico_dates_end_ramp['SSP5-8.5']+1,2300+1))] = 0. # 0 afterwards
