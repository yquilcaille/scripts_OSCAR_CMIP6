...
from core_fct.fct_loadD import load_all_hist , load_emissions_scen,load_landuse_scen
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'hist-GHG':
## - Concentrations for CO2, CH4, N2O and halo from 'concentrations_CMIP6' and 'concentrations_ScenarioMIP' over 2015-2020
...
with xr.open_dataset('input_data/observations/concentrations_CMIP6.nc') as TMP:
    for var in ['CO2','CH4','N2O']:
        ...
        For['D_'+var].loc[{'year':np.arange(year_start,2014+1)}] = TMP[var].loc[{'year':np.arange(year_start,2014+1),'region':'Globe'}]-Par[var+'_0']
    For['D_Xhalo'] = xr.DataArray( np.full( fill_value=np.nan , shape=(year_end-year_start+1,len(For.spc_halo))), dims=('year','spc_halo') )
    val = TMP['Xhalo'].loc[{'year':np.arange(year_start,2014+1),'region':'Globe'}]-Par['Xhalo_0']
    For['D_Xhalo'].loc[{'year':np.arange(year_start,2014+1),'spc_halo':val.spc_halo}] = val
...
with xr.open_dataset('input_data/drivers/concentrations_ScenarioMIP.nc') as TMP:
    for var in ['CO2','CH4','N2O','Xhalo']:
        val = TMP[var].loc[{'year':np.arange(2015,year_end+1),'scen':'SSP2-4.5'}] - Par[var+'_0']
        if 'spc_halo' in val.dims:
            For['D_'+var].loc[{'year':np.arange(2015,year_end+1),'spc_halo':val.spc_halo}] = val
        else:
            For['D_'+var].loc[{'year':np.arange(2015,year_end+1)}] = val
