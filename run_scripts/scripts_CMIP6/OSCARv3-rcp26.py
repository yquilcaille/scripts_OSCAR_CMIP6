...
out_init = xr.open_dataset('results/'+folder+'/historical-CMIP5_Out-'+str(setMC)+'.nc' )
...
##################################################
## Forcings for 'rcp26':
## - Concentrations for CO2, CH4, N2O and halo: from 'concentrations_Meinshausen2011', RCP2.6
## - Emissions for all from 'emissions_RCPdb', RCP2.6  (FF, N2O, CH4 and Xhalo: not used, but here to allow the computation to run)
## - LULCC from LUH1, RCP2.6.
...
for_runs_hist = xr.open_dataset('results/'+folder+'/historical-CMIP5_For-'+str(setMC)+'.nc')
...
with xr.open_dataset('input_data/drivers/concentrations_Meinshausen_2011.nc') as TMP:
    ...
    for var in ['CO2','CH4','N2O','Xhalo']:
        ...
        vals = TMP[var].loc[{'year':np.arange(2005+1,year_end+1),'scen':'RCP2.6'}] - Par[var+'_0']
...
for var in ['E_BC','E_CO','E_NH3','E_VOC','E_NOX','E_OC','E_SO2'] + ['Eff','E_CH4','E_N2O']:
    For[var].loc[{'year':np.arange(year_start+1,2100+1)}] = For0E[var].loc[{'year':np.arange(year_start+1,2100+1),'scen_'+var:'RCP2.6'}]
    if var == 'Eff':
        For[var].loc[{'year':np.arange(2100+1,year_end+1),'reg_land':0}] = For0E2[var].loc[{'year':np.arange(2100+1,year_end+1),'scen':'RCP2.6'}]
    else:
...
For['E_Xhalo'].loc[{'year':np.arange(year_start+1,2100+1),'spc_halo':For0E.spc_halo}] = For0E['E_Xhalo'].loc[{'year':np.arange(year_start+1,2100+1),'scen_E_Xhalo':'RCP2.6'}]
For['E_Xhalo'].loc[{'year':np.arange(2100+1,year_end+1),'spc_halo':For0E2.spc_halo,'reg_land':0}] = For0E2['E_Xhalo'].loc[{'year':np.arange(2100+1,year_end+1),'scen':'RCP2.6'}]
...
For['d_Hwood'].loc[{'year':np.arange(2004+1,2099+1),'bio_land':For0L.bio_land}] = For0L['d_Hwood'].loc[{'year':np.arange(2004+1,2099+1),'scen_LULCC':'RCP2.6'}]
...
for var in ['d_Ashift','d_Acover']:
    ...
    For[var].loc[{'year':np.arange(2004+1,2099+1),'bio_from':For0L.bio_from,'bio_to':For0L.bio_to}] = For0L[var].loc[{'year':np.arange(2004+1,2099+1),'scen_LULCC':'RCP2.6'}]
...
with xr.open_dataset('input_data/drivers/radiative-forcing_Meinshausen_2011.nc') as TMP:
    for tp_rf in ['volc','solar']:
        ...
        For['RF_'+tp_rf].loc[{'year':np.arange(2005+1,year_end+1)}] = TMP['RF_'+tp_rf].loc[{'scen':'RCP2.6','year':np.arange(2005+1,year_end+1)}]
    # For = For.drop('scen')


for tp_rf in ['volc','solar']:
    For['RF_'+tp_rf] = For['RF_'+tp_rf].fillna( 0. )
## RF contr
