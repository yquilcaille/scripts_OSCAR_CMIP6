...
out_init = xr.open_dataset('results/'+folder+'/hist-CO2_Out-'+str(setMC)+'.nc' )
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'ssp245-CO2':
## - Concentrations for CO2 from 'concentrations_ScenarioMIP' over 2021-2100
...
for_runs_hist = xr.open_dataset('results/'+folder+'/hist-CO2_For-'+str(setMC)+'.nc' )
...
with xr.open_dataset('input_data/drivers/concentrations_ScenarioMIP.nc') as TMP:
    ## Concentrations CO2
    For['D_CO2'] = TMP['CO2'].loc[{'year':np.arange(year_start,year_end+1),'scen':'SSP2-4.5'}] - Par['CO2_0']
    For = For.drop('scen')
