...
out_init = xr.open_dataset('results/'+folder+'/esm-ssp585_Out-'+str(setMC)+'.nc' )
...
## *********************************************************
## - LULCC from LUH2, extension for SSP5-8.5
...
for_runs_scen = xr.open_dataset('results/'+folder+'/esm-ssp585_For-'+str(setMC)+'.nc')
...
for var in ['d_Hwood','d_Ashift','d_Acover']:
    For[var].loc[{'year':np.arange(year_start,2299+1)}] = For0L[var].loc[{'year':np.arange(year_start,2299+1),'scen_LULCC':'SSP5-8.5'}]
