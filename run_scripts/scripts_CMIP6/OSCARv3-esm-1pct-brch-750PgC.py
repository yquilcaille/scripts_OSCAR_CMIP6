...
name_experiment = 'esm-1pct-brch-750PgC'
year_PI = 1850
year_start = 1850
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'esm-1pct-brch-750PgC':
## - Concentrations for CH4, N2O and halo from 'concentrations_CMIP6', year 1850
## - Emissions of CO2 as compatible emissions, but cut when 750 PgC have been emitted
...
if False:
    ## comparing for 1pctCO2 effect of Foxi_CH4 that has to be run before
    for VAR in ['D_Ewet','D_Ebb','D_Epf_CH4','D_Fsink_CH4']:
        out_conc[VAR] = OSCAR[VAR](out_conc, Par, for_conc,recursive=True)
    E_CH4 = Par.a_CH4*out_conc['D_CH4'].diff('year') - (out_conc['D_Ewet'] + out_conc['D_Ebb'].sel({'spc_bb':'CH4'}, drop=True).sum('bio_land')).sum('reg_land') - out_conc['D_Epf_CH4'].sum('reg_pf') + out_conc['D_Fsink_CH4']
    Foxi = 1E-3 * (Par.p_CH4geo * E_CH4 + out_conc['D_Epf_CH4'].sum('reg_pf')  - out_conc['D_CH4'].dropna('year').differentiate('year'))
    for VAR in ['D_Eluc','D_Epf_CO2','D_Fland','D_Focean']:
        out_conc[VAR] = OSCAR[VAR](out_conc, Par, for_conc,recursive=True)
    ## calculating compatible emissions
    val = - out_conc.D_Eluc  -  out_conc.D_Epf_CO2.sum('reg_pf',min_count=1)  +  out_conc.D_Fland  +  out_conc.D_Focean  -  Foxi
    For['Eff'].loc[{'reg_land':0,'year':np.arange(year_start+1,for_conc.year.isel(year=-1)+1)}] = Par.a_CO2 * out_conc.D_CO2.diff(dim='year') + 0.5*( val + val.shift(year=1) )

    plt.plot( np.arange(year_start,for_conc.year.isel(year=-1)+1) , For['Eff'].sel(reg_land=0,year=np.arange(year_start,for_conc.year.isel(year=-1)+1)) * list_noDV[name_experiment][setMC] , color='r',ls='--' )
    plt.plot( np.arange(year_start,for_conc.year.isel(year=-1)+1) , For['Eff'].sel(config=0,reg_land=0,year=np.arange(year_start,for_conc.year.isel(year=-1)+1)) * list_noDV[name_experiment][setMC][:,0] , color='r',ls='--' , label="withOUT recursive call for Foxi_CH4" )

    plt.title('Compatible Eff under 1pctCO2, removing diverging runs')
    plt.legend(loc=0)


## calculating year exceeding required cumulated compatible emissions
cum = For.Eff.sel(reg_land=0).cumsum('year')
yy = For.year.isel(year=(cum<750.).argmin('year')) # last year where exceeds this threshold. It is 0 if does not reach it.
...
for ic in configs_reached:
    ...
    For['Eff'].loc[{'reg_land':0,'year':For['year_breach'].isel(config=ic),'config':For.config.isel(config=ic)}] = 750. - cum.loc[{'year':For['year_breach'].isel(config=ic)-1,'config':For.config.isel(config=ic)}]
## Checking:
if False:
