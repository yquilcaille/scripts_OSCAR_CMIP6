...
name_experiment = 'land-hist-altLu1'
year_PI = 1850
year_start = 1850
year_end = 2010
nMC = 500
...
out_init = xr.open_dataset('results/'+folder+'/land-spinup-altLu1_Out-'+str(setMC)+'.nc' )
...
## - Land-use: LUH2, reconstruction over historical "LUH2-High"
...
for var in ['d_Ashift','d_Acover','d_Hwood']:
    For[var] = For0[var].loc[{'year':np.arange(year_start,year_end+1),'data_LULCC':'LUH2-High'}]
For = For.drop('data_LULCC')

## CORRECTION OF 'Par.Aland_0': accounting for different starting year
for_init = xr.open_dataset('results/'+folder+'/land-spinup-altLu1_For-'+str(setMC)+'.nc' )
