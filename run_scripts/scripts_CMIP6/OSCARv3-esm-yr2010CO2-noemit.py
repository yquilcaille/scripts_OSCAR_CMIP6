...
name_experiment = 'esm-yr2010CO2-noemit'
year_PI = 1850
year_start = 1850
year_end = 2010+1000
nMC = 1000
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'esm-yr2010CO2-noemit':
## - Concentrations of CH4, N2O and halo are still driven using 'concentrations_CMIP6'
## - Emissions
##      - CO2 from compatible emissions from the experiment 'yr2010CO2'. From 2015, these emissions are set to 0.
...
## compatible emissions up to 2015
val = - out_conc.D_Eluc  -  out_conc.D_Epf_CO2.sum('reg_pf',min_count=1)  +  out_conc.D_Fland  +  out_conc.D_Focean  -  out_conc.D_Foxi_CH4
For['Eff'].loc[{'reg_land':0,'year':np.arange(year_start+1,2015+1)}] = (Par.a_CO2 * out_conc.D_CO2.diff(dim='year') + 0.5*( val + val.shift(year=1) )).loc[{'year':np.arange(year_start+1,2015+1)}]
