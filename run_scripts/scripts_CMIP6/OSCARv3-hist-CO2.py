...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'hist-CO2':
## - Concentrations for CO2 from 'concentrations_CMIP6' over 1850-2014 and 'concentrations_ScenarioMIP' over 2015-2020
...
with xr.open_dataset('input_data/observations/concentrations_CMIP6.nc') as TMP:
    ...
    ## Concentrations CO2 1850-2014
    For['D_CO2'] = xr.DataArray( np.full( fill_value=np.nan , shape=(year_end-year_start+1)), dims=('year') )
    For['D_CO2'].loc[{'year':np.arange(year_start,2014+1)}] = TMP['CO2'].loc[{'year':np.arange(year_start,2014+1),'region':'Globe'}]-Par['CO2_0']
    For = For.drop('region')
with xr.open_dataset('input_data/drivers/concentrations_ScenarioMIP.nc') as TMP:
    ## Concentrations CO2 2015-2020
    For['D_CO2'].loc[{'year':np.arange(2015,year_end+1)}] = TMP['CO2'].loc[{'year':np.arange(2015,year_end+1),'scen':'SSP2-4.5'}] - Par['CO2_0']
