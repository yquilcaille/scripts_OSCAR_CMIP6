...
name_experiment = 'esm-ssp585-ssp126Lu-ext'
...
## 3. INITIALIZATION
##################################################
## Using initialization from last year of ssp585
out_init = xr.open_dataset('results/'+folder+'/esm-ssp585-ssp126Lu_Out-'+str(setMC)+'.nc' )
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'esm-ssp585-ssp126Lu-ext':
## - Concentrations for CH4, N2O and halo: from 'concentrations_ScenarioMIP', SSP5-8.5-ext
## *********************************************************
...
for_runs_scen = xr.open_dataset('results/'+folder+'/esm-ssp585-ssp126Lu_For-'+str(setMC)+'.nc')
...
if SOURCE_EXTENSION=='ONeill2017': # O'Neill et al, 2017 (doi:10.5194/gmd-9-3461-2016)
    ...
    for var in ['E_BC','E_CO','E_NH3','E_VOC','E_NOX','E_OC','E_SO2'] + ['E_CH4']:
        ## non-CO2 emissions from LU AND FF&I stay constant after 2100
        cst_emi = TMP[var].loc[dict(scen='SSP5-8.5',year=2100,sector=[sec for sec in TMP.sector.values if sec not in nonCO2_ignored_sectors])].sum('sector')
    ...
    cst_emi = TMP['E_N2O'].loc[dict(scen='SSP5-8.5',year=2100)]
    ...
    if TMP['Eff'].loc[dict(scen='SSP5-8.5',year=2100,sector=[sec for sec in TMP.sector.values if sec in sectors_LU])].sum() > 0.: raise Exception("Check definition of the sectors.")
    todec_emiFF = TMP['Eff'].loc[dict(scen='SSP5-8.5',year=2100,sector=[sec for sec in TMP.sector.values if sec not in nonCO2_ignored_sectors])].sum('sector')
    ...
    for var in ['E_BC','E_CO','E_NH3','E_VOC','E_NOX','E_OC','E_SO2'] + ['E_CH4']:
        ## non-CO2 emissions from LU stay constant after 2100
        cst_emiLU = TMP[var].loc[dict(scen='SSP5-8.5',year=2100,sector=sectors_LU)].sum('sector')
        For[var] = For[var].fillna( cst_emiLU )
        ## non-CO2 emissions from FF&I decrease linearly to  0 over 2100-2250
        todec_emiFF = TMP[var].loc[dict(scen='SSP5-8.5',year=2100,sector=[sec for sec in TMP.sector.values if sec not in nonCO2_ignored_sectors+sectors_LU])].sum('sector')
    ...
    todec_emiall = TMP['E_N2O'].loc[dict(scen='SSP5-8.5',year=2100)]
    ...
    For['Eff'].loc[dict(year=range(year_start,dico_dates_end_plateau['SSP5-8.5']+1))] = todec_emiFF # constant over 2100-2140
    For['Eff'].loc[dict(year=np.arange(dico_dates_end_plateau['SSP5-8.5'],dico_dates_end_ramp['SSP5-8.5']+1))] = np.linspace( todec_emiFF , 0.*todec_emiFF , dico_dates_end_ramp['SSP5-8.5']-dico_dates_end_plateau['SSP5-8.5']+1)  # linear ramp over 2140-2190
    For['Eff'].loc[dict(year=np.arange(dico_dates_end_ramp['SSP5-8.5']+1,year_end+1))] = 0. # 0 afterwards
