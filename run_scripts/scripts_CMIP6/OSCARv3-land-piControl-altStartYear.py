...
name_experiment = 'land-piControl-altStartYear'
year_PI = 1700
year_start = 1700
year_end = 1700+1000
nMC = 500
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'land-piControl-altStartYear':
## - Concentration for CO2: concentrations CMIP6, 1700 value
## - Climatology (local temperatures and precipitations): GSWP3 recycled over 1901-1920
## - Land-use: LUH2 historical, fixed at 1700 values
...
with xr.open_dataset('input_data/observations/climatology_GSWP3.nc') as TMP:
    ...
    for var in ['Tl','Pl']:
        ...
        For['D_'+var].loc[{'year':np.arange(year_start,year_start+int((year_end-year_start)/20.)*20)}] = np.stack( list(val.sel(year=np.arange(1901,1920+1)).values) * int((year_end-year_start)/20.) )
        For['D_'+var].loc[{'year':np.arange(year_start+int((year_end-year_start)/20.)*20,year_end+1)}] = val.isel(year=np.arange(year_end-year_start-int((year_end-year_start)/20.)*20+1)).values
