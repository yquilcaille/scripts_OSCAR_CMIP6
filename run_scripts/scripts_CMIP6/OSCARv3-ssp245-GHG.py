...
out_init = xr.open_dataset('results/'+folder+'/hist-GHG_Out-'+str(setMC)+'.nc' )
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'ssp245-GHG':
## - Concentrations for CO2, CH4, N2O and halo from 'concentrations_ScenarioMIP'
...
for_runs_hist = xr.open_dataset('results/'+folder+'/hist-GHG_For-'+str(setMC)+'.nc' )
...
with xr.open_dataset('input_data/drivers/concentrations_ScenarioMIP.nc') as TMP:
    for var in ['CO2','CH4','N2O','Xhalo']:
        For['D_'+var] = TMP[var].loc[{'year':np.arange(year_start,year_end+1),'scen':'SSP2-4.5'}] - Par[var+'_0']
    For.drop('scen')
