...
out_init = xr.open_dataset('results/'+folder+'/hist-volc_Out-'+str(setMC)+'.nc' )
...
## - RF for volcanoes as the ramp over 10 years from ssp245, using reference CMIP6 
...
for_runs_hist = xr.open_dataset('results/'+folder+'/hist-volc_For-'+str(setMC)+'.nc' )
...
For['RF_volc'].loc[dict(year=np.arange(year_start,2014+10+1))] = np.linspace(tmp_start,tmp_end,10+1)[year_start-2014:]  # ramp over 10 years
