...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'hist-stratO3' (similar to piControl, except D_O3s):
...
## - Stratospheric ozone forced to outputs of OSCAR from the experiment 'historical' over 1850-2014, and from 'ssp245' over 2015-2020
...
## Stratospheric ozone
For['D_O3s'] = xr.DataArray( np.full( fill_value=np.nan , shape=(year_end-year_start+1,nMC)), dims=('year','config') )
## 1850-2014
for_hist = xr.open_dataset('results/'+folder+'/historical_For-'+str(setMC)+'.nc' )
out_hist = xr.open_dataset('results/'+folder+'/historical_Out-'+str(setMC)+'.nc' )
out_hist['D_O3s'] = OSCAR['D_O3s'](out_hist, Par, for_hist,recursive=True)
For['D_O3s'].loc[{'year':np.arange(year_start,2014+1)}] = out_hist.D_O3s.sel(year=np.arange(year_start,2014+1))
## 2015-2020
for_scen = xr.open_dataset('results/'+folder+'/ssp245_For-'+str(setMC)+'.nc' )
out_scen = xr.open_dataset('results/'+folder+'/ssp245_Out-'+str(setMC)+'.nc' )
out_scen['D_O3s'] = OSCAR['D_O3s'](out_scen, Par, for_scen,recursive=True)
For['D_O3s'].loc[{'year':np.arange(2015,year_end+1)}] = out_scen.D_O3s.sel(year=np.arange(2015,year_end+1))
