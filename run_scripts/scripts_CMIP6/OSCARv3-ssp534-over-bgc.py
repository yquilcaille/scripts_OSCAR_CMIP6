...
name_experiment = 'ssp534-over-bgc'
year_PI = 1850
year_start = 2014
year_end = 2100
nMC = 1000
...
## 3. INITIALIZATION
##################################################
## Using initialization from last year of hist-bgc
out_init = xr.open_dataset('results/'+folder+'/hist-bgc_Out-'+str(setMC)+'.nc' )
...
## - Emissions for all from 'emissions_ScenarioMIP', SSP5-3.4-OS  (FF, N2O, CH4 and Xhalo: not used, but here to allow the computation to run)
## - LULCC from LUH2, SSP5-3.4 (NB: not called '-OS')
...
for var in ['E_BC','E_CO','E_NH3','E_VOC','E_NOX','E_OC','E_SO2'] + ['Eff','E_CH4','E_N2O']:
    For[var].loc[{'year':np.arange(year_start+1,year_end+1)}] = For0E[var].loc[{'year':np.arange(year_start+1,year_end+1),'scen_'+var:'SSP5-3.4-OS'}]
...
For['d_Hwood'].loc[{'year':np.arange(year_start+1,year_end),'bio_land':For0L.bio_land}] = For0L['d_Hwood'].loc[{'year':np.arange(year_start+1,year_end),'scen_LULCC':'SSP5-3.4'}]
For['d_Hwood'].loc[dict(year=year_end)] = For['d_Hwood'].sel(year=year_end-1) # 2100 as 2099
for var in ['d_Ashift','d_Acover']:
    For[var].loc[{'year':np.arange(year_start+1,year_end),'bio_from':For0L.bio_from,'bio_to':For0L.bio_to}] = For0L[var].loc[{'year':np.arange(year_start+1,year_end),'scen_LULCC':'SSP5-3.4'}]
