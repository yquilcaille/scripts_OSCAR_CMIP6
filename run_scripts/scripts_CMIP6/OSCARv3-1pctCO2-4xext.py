...
name_experiment = '1pctCO2-4xext'
year_PI = 1850
year_start = 1850
year_end = 1850+140+1000
nMC = 1000
setMC = 1
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for '1pctCO2-4xext':
## - Concentrations for CO2: from the value in 1850, increase by 1%/yr of the atmospheric CO2 (1850: CO2_1850, 1851:*1.01, 1852: *1.01**2.,...). Lasts for 140 years, with 1%/yr applied throughout, but branch at 140 years: last year is exactly 4x PI, and extension over 1000 years.
...
with xr.open_dataset('input_data/observations/concentrations_CMIP6.nc') as TMP:
    ...
    For['D_CO2'].loc[{'year':year_start}] = TMP['CO2'].loc[{'year':year_PI,'region':'Globe'}] - Par['CO2_0']
    For['D_CO2'].loc[{'year':np.arange(year_start+1,year_start+140+1)}] = xr.DataArray( np.cumprod(np.repeat(1.01,140)),coords={'year':np.arange(year_start+1,year_start+140+1)},dims=('year')) * TMP['CO2'].loc[{'year':year_PI,'region':'Globe'}] - Par['CO2_0']
    For['D_CO2'].loc[{'year':np.arange(year_start+140,year_end+1)}] = 4.*TMP['CO2'].loc[{'year':year_PI,'region':'Globe'}] - Par['CO2_0']
    ## CH4 and N2O
    for var in ['CH4','N2O']:
