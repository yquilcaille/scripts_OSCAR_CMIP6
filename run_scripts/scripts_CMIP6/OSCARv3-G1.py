...
year_start = 1850
year_end = 1850+1000
print('Warning, this experiment is directly G1 AND G1ext.')
nMC = 1000
setMC = 1
...
##################################################
## Forcings for 'G1':
## - Concentrations for CO2: value of 1850 multiplied by 4 in 1850, and held over time.
...
## - RF for solar as reference CMIP6, year 1850, to which we withdraw RF_CO2(4xCO2_0)-RF_CO2(CO2_0)
## - RF for contrails: 0
...
with xr.open_dataset('input_data/observations/concentrations_CMIP6.nc') as TMP:
    ...
    For['D_CO2'] = For['D_CO2'].fillna( 4. * TMP['CO2'].loc[{'year':year_PI,'region':'Globe'}] - Par['CO2_0'] )
    ## CH4 and N2O
    for var in ['CH4','N2O']:
...
For['RF_solar'] -= Par.rf_CO2 * (np.log1p( For.D_CO2.isel(year=0) / Par.CO2_0) - np.log1p( (TMP['CO2'].loc[{'year':year_PI,'region':'Globe'}] - Par['CO2_0']) / Par.CO2_0))
...
if False:
    val = OSCAR['RF_CO2'](Out, Par, For)
    plt.plot( val.year , val+For.RF_solar )
