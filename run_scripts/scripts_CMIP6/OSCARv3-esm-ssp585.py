...
from core_fct.fct_loadD import load_emissions_scen,load_landuse_scen  # bug with 'load_all_scen'
...
for tp_rf in ['volc','solar']:
    tmp_start = for_runs_hist['RF_'+tp_rf].loc[{'year':2014}]
    tmp_end = for_runs_hist['RF_'+tp_rf].loc[{'year':np.arange(1850,2014+1)}].mean('year')
    For['RF_'+tp_rf].loc[dict(year=np.arange(year_start,year_start+10))] = np.linspace(tmp_start,tmp_end,10+1)[1:]  # ramp over 10 years
