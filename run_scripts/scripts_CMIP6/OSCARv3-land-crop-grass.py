...
name_experiment = 'land-crop-grass'
year_PI = 1850
year_start = 1850
year_end = 2010
nMC = 500
...
## - Land-use: LUH2 historical, but all new crop and pasture-land treated as unmanaged grassland
...
## Transfering new Croplands and Pastures to Non-Forested
For['d_Hwood'].loc[{'bio_land':'Non-Forest'}] = For['d_Hwood'].loc[{'bio_land':['Non-Forest','Cropland','Pasture']}].sum('bio_land')
for var in ['d_Ashift','d_Acover']:
    val = For[var].loc[{'bio_from':['Non-Forest','Cropland','Pasture'],'bio_to':['Non-Forest','Cropland','Pasture']}].sum(('bio_from','bio_to'))
    For[var].loc[{'bio_from':['Non-Forest','Cropland','Pasture'],'bio_to':['Non-Forest','Cropland','Pasture']}] = 0.
    For[var].loc[{'bio_from':'Non-Forest','bio_to':'Non-Forest'}] = val
    for b_i in ['Forest', 'Urban']:# 'Cropland', 'Non-Forest', 'Pasture'
        For[var].loc[{'bio_from':'Non-Forest','bio_to':b_i}] = For[var].loc[{'bio_from':['Non-Forest','Cropland','Pasture'],'bio_to':b_i}].sum('bio_from')
        For[var].loc[{'bio_from':['Cropland','Pasture'],'bio_to':b_i}] = 0.
        For[var].loc[{'bio_to':'Non-Forest','bio_from':b_i}] = For[var].loc[{'bio_to':['Non-Forest','Cropland','Pasture'],'bio_from':b_i}].sum('bio_to')
        For[var].loc[{'bio_to':['Cropland','Pasture'],'bio_from':b_i}] = 0.
