...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'land-cCO2':
## - Concentration for CO2: concentrations CMIP6, frozen to 1850 level
...
with xr.open_dataset('input_data/observations/concentrations_CMIP6.nc') as TMP:
    for var in ['CO2']:
        For['D_'+var] = xr.DataArray( np.full( fill_value=TMP[var].loc[{'year':year_PI,'region':'Globe'}] - Par[var+'_0'] , shape=(year_end-year_start+1)), dims=('year') )
