...
##################################################
## Forcings for 'ssp126':
## - Concentrations for CH4, N2O and halo: from 'concentrations_ScenarioMIP', SSP1-2.6
## - Emissions for all from 'emissions_ScenarioMIP', SSP1-2.6  (N2O, CH4 and Xhalo: not used, but here to allow the computation to run). Eff fall in this category
...
for var in ['E_BC','E_CO','E_NH3','E_VOC','E_NOX','E_OC','E_SO2'] + ['Eff','E_CH4','E_N2O']:
    For[var].loc[{'year':np.arange(year_start+1,year_end+1)}] = For0E[var].loc[{'year':np.arange(year_start+1,year_end+1),'scen_'+var:'SSP1-2.6'}]
