...
##      - the carbon cycle DOES NOT RESPOND to the increase in atmospheric CO2
##      - the climate system responds to the increase in atmospheric CO2, and the CO2 of 1850 is prescribed instead.
...
## 5. RUN
##################################################
## Updating parameters so that a constant atm CO2 will be prescribed to the carbon cycle
Par['D_CO2_bgc'] = For.D_CO2.sel(year=year_start)
...
if False:
    Out_no_bgcrad = OSCAR(Ini, Par, For )

    Par['D_CO2_rad'] = For.D_CO2.sel(year=year_start)
    Out_bgc = OSCAR(Ini, Par, For )
    Par = Par.drop('D_CO2_rad')

    Par['D_CO2_bgc'] = For.D_CO2.sel(year=year_start)
    Out_rad = OSCAR(Ini, Par, For )
    Par = Par.drop('D_CO2_bgc')

    plt.figure()
    plt.subplot(121)
    plt.title('D_Tg')
    plt.plot( Out_no_bgcrad.year , Out_no_bgcrad.D_Tg.mean('config') , label='BGC&RAD')
    plt.plot( Out_bgc.year , Out_bgc.D_Tg.mean('config') , label='BGC' )
    plt.plot( Out_rad.year , Out_rad.D_Tg.mean('config')  , label='RAD')
    plt.grid()
    plt.legend(loc=0)

    plt.subplot(122)
    plt.title('D_cveg')
    plt.plot( Out_no_bgcrad.year , Out_no_bgcrad.sum(('reg_land','bio_land')).D_cveg.mean('config') , label='BGC&RAD')
    plt.plot( Out_bgc.year , Out_bgc.D_cveg.sum(('reg_land','bio_land')).mean('config') , label='BGC' )
    plt.plot( Out_rad.year , Out_rad.D_cveg.sum(('reg_land','bio_land')).mean('config')  , label='RAD')
    plt.grid()
    plt.legend(loc=0)
