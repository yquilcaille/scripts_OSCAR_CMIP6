...
name_experiment = 'esm-1pct-brch-1000PgC'
year_PI = 1850
year_start = 1850
...
## 4. DRIVERS / FORCINGS
##################################################
## Forcings for 'esm-1pct-brch-1000PgC':
## - Concentrations for CH4, N2O and halo from 'concentrations_CMIP6', year 1850
## - Emissions of CO2 as compatible emissions, but cut when 1000 PgC have been emitted
...
yy = For.year.isel(year=(cum<1000.).argmin('year')) # last year where exceeds this threshold. It is 0 if does not reach it.
...
for ic in configs_reached:
    ...
    For['Eff'].loc[{'reg_land':0,'year':For['year_breach'].isel(config=ic),'config':For.config.isel(config=ic)}] = 1000. - cum.loc[{'year':For['year_breach'].isel(config=ic)-1,'config':For.config.isel(config=ic)}]
## Checking:
if False:
