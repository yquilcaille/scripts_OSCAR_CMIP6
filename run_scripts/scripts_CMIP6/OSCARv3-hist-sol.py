...
## - RF for solar as reference CMIP6 over 1850-2014 + the ramp over 10 years from ssp245
...
## RF solar
For['RF_solar'] = xr.DataArray( np.full(fill_value=np.nan , shape=[year_end-year_start+1]), dims=['year'] )
## historical evolutions up to 2014
For['RF_solar'].loc[{'year':np.arange(year_start,2014+1)}] = For0['RF_solar'].loc[{'year':np.arange(year_start,2014+1),'data_RF_solar':'CMIP6'}]
## ramp down over 10 years to background level, as prescribed in ssp245
tmp_start = For0['RF_solar'].loc[{'year':2014,'data_RF_solar':'CMIP6'}]
tmp_end = For0['RF_solar'].loc[{'year':np.arange(1850,2014+1),'data_RF_solar':'CMIP6'}].mean('year')
For['RF_solar'].loc[dict(year=np.arange(2014+1,min([2014+10,year_end])+1))] = np.linspace(tmp_start,tmp_end,10+1)[1:min([2014+10,year_end+1])-2014]  # ramp over 10 years
For['RF_solar'] = For['RF_solar'].fillna( tmp_end )
